...
const loadDataDashboard = (data) => {
    ...
    sawroomDB.collection("dashboard").doc("operators").get().then(function(doc){
        const allOperators = doc.data().allOperators;
        const firstOperator = document.createElement("select");
        const secondOperator = document.createElement("select");
        const defaultOption = document.createElement("option");
        defaultOption.textContent = "--";

        firstOperator.className = "operator firstOperator";
        firstOperator.appendChild(defaultOption.cloneNode(true));
        for (let i = 0; i < allOperators.length; i++) {
            let option = document.createElement("option");
            option.textContent = allOperators[i];
            option.id = "op1";
            firstOperator.appendChild(option);
        }
        secondOperator.className = "operator secondOperator";
        secondOperator.appendChild(defaultOption.cloneNode(true));
        for (let i = 0; i < allOperators.length; i++) {
            let option = document.createElement("option");
            option.textContent = allOperators[i];
            option.id = "op2";
            secondOperator.appendChild(option);
        }
            ...
            element.addEventListener("click", function() {
                ...
                confirmDialog.children[0].children[1].addEventListener("click", function() {
                    privateDocRef.update(
                        {
                            firstOperator: firstOperator.outerHTML,
                            secondOperator: secondOperator.outerHTML
                        })
                    setTimeout(() => {
                ...
                docRef.get().then(function(doc) {
                    if (doc.exists) {
                        ...
                        function confirmEditBooking() {
                            ...
                            sawroomDB.collection("reservations").doc((new Date(new Date(document.querySelector(".editInputDate").value).toLocaleDateString("en-US") + " " + document.querySelector(".editInputTime").value).getTime() + 7200000).toString()).set({
                                booking: new Date(new Date(document.querySelector(".editInputDate").value).toLocaleDateString("en-US") + " " + document.querySelector(".editInputTime").value).getTime() + 7200000
                            })
                            sawroomDB.collection("reservationsSecure").doc((new Date(new Date(document.querySelector(".editInputDate").value).toLocaleDateString("en-US") + " " + document.querySelector(".editInputTime").value).getTime() + 7200000).toString()).set({
                                booking: new Date(new Date(document.querySelector(".editInputDate").value).toLocaleDateString("en-US") + " " + document.querySelector(".editInputTime").value).getTime() + 7200000,
                            ...
                            this.parentNode.parentNode.innerHTML = `
                                <td>${new Date(new Date(new Date(document.querySelector(".editInputDate").value).toLocaleDateString("en-US") + " " + document.querySelector(".editInputTime").value).getTime() + 7200000).toLocaleDateString('en-GB', options)}</td>
...
const editTeam = create("div");
editTeam.className = "editTeam";
editTeam.innerHTML = `<span><i class="fas fa-user-cog"></i> Edit Team</span>`;
...
sawroomDB.collection("dashboard").doc("operators").get().then(function(doc){
    const allOperators = doc.data().allOperators;
    const allOperatorsDiv = doc.data().allOperatorsDiv;
    const operatorColors = doc.data().operatorColors;
    for (let i = 0; i < allOperators.length; i++) {
    ...
    editTeam.addEventListener("click", function() {
        const confirmDialog = document.querySelector(".confirmDialog");
            ...
            confirmDialog.innerHTML = `
            <div class="container">
                <div class="heading">Edit Team</div>
                <div class="editTeamOperatorsAndColors"></div>
                <div class="addOperator">
                    <input type="color" class="colorInput">
                    <input type="text" placeholder="Add Operator" class="addOperatorInput">
                    <i style="color: red" class="fas fa-plus addOperatorButton"></i>
                </div>
                <div class="moneyFromGroup">
                    <div class="text">Money From Group:</div>
                    <input type="text" value="${doc.data().moneyFromGroup}" class="moneyFromGroupInput">
                </div>
                <div style="margin-top: 20px;" class="decline">Close</div>
            </div>
            `;
        document.querySelector(".colorInput").value = '#'+Math.floor(Math.random()*16777215).toString(16);
        document.querySelector(".decline").addEventListener("click", function() {
            confirmDialog.style.display = "none";
        })
        document.querySelector(".editTeamOperatorsAndColors").innerHTML = allOperatorsDiv;
        for (let i = 0; i < operatorColors.length; i++) {
            document.querySelectorAll(".inputColor")[i].value = operatorColors[i];
        }
        for (let i = 0; i < document.querySelector(".editTeamOperatorsAndColors").childNodes.length; i++) {
            if (document.querySelector(".editTeamOperatorsAndColors").childNodes[i].textContent == ",") {
                document.querySelector(".editTeamOperatorsAndColors").childNodes[i].textContent = ""
            }
        }
        document.querySelectorAll(".inputColor").forEach(color => {
            color.addEventListener("change", function() {
                this.parentElement.style.backgroundColor = this.value;
            })
        })
        document.querySelectorAll("div.deleteButton .fa-trash").forEach(button => {
            button.addEventListener("click", function() {
                let docRef = sawroomDB.collection("dashboard").doc("operators");
                docRef.update({
                    "allOperators": firebase.firestore.FieldValue.arrayRemove(this.parentElement.parentElement.childNodes[0].childNodes[1].textContent),
                    "allOperatorsDiv": firebase.firestore.FieldValue.arrayRemove(`<div class="teamMember"><div><div class="operatorColor"><input type="color" class="inputColor"></div><div class="operator">${this.parentElement.parentElement.childNodes[0].childNodes[1].textContent}</div></div><div class="deleteButton"><i class="fas fa-trash"></i></div></div>`),
                    "operatorColors": firebase.firestore.FieldValue.arrayRemove(this.parentElement.parentElement.childNodes[0].childNodes[0].childNodes[0].value)
                })
                this.parentElement.parentElement.style.display = "none";
            })
        })
        document.querySelector(".addOperatorButton").addEventListener("click", function() {
            let docRef = sawroomDB.collection("dashboard").doc("operators");
            if(this.previousElementSibling.value !== "") {
                docRef.update({
                    "allOperators": firebase.firestore.FieldValue.arrayUnion(this.previousElementSibling.value),
                    "allOperatorsDiv": firebase.firestore.FieldValue.arrayUnion(`<div class="teamMember"><div><div class="operatorColor"><input type="color" class="inputColor"></div><div class="operator">${this.previousElementSibling.value}</div></div><div class="deleteButton"><i class="fas fa-trash"></i></div></div>`),
                    "operatorColors": firebase.firestore.FieldValue.arrayUnion(this.previousElementSibling.previousElementSibling.value)
                })
            }
        })
        document.querySelector(".moneyFromGroupInput").addEventListener("keyup", function() {
            sawroomDB.collection("dashboard").doc("operators").update({"moneyFromGroup": parseInt(this.value)})
        })
    })
})
...
const loadDataMonthlyTables = (data) => {
    ...
    sawroomDB.collection("dashboard").doc("operators").get().then(function(doc){
        ...
        for (let i = 0; i < allOperators.length; i++) {
            groupsCounter.push(1);
            moneyCounter.push(0);
        }

        document.querySelectorAll(".totalMoney").forEach(money => {
            if(money.innerHTML == "") {
                money.innerHTML = "0";
            }
        })
        document.querySelectorAll(".totalGroups").forEach(groups => {
            if(groups.innerHTML == "") {
                groups.innerHTML = "0 групи";
            }
        })
    })
    // Missing Code
    ...
    sawroomDB.collection("dashboard").doc("operators").get().then(function(doc){
        const allOperators = doc.data().allOperators;
        const operatorColors = doc.data().operatorColors;
        const moneyFromGroup = doc.data().moneyFromGroup;
        document.querySelectorAll(".operatorText").forEach((element) => {
            for (let i = 0; i < allOperators.length; i++) {
                document.querySelectorAll(".color")[i].style.backgroundColor = operatorColors[i];
                if (element.textContent == allOperators[i]) {
                    document.querySelectorAll(".totalGroups")[i].textContent = `${groupsCounter[i]++} групи`;
                    document.querySelectorAll(".totalMoney")[i].textContent = `${moneyCounter[i] = moneyCounter[i] + moneyFromGroup}`;

                    if (document.querySelectorAll(".totalGroups")[i].textContent == "1 групи") {
                        document.querySelectorAll(".totalGroups")[i].textContent = "1 група";
                    }
                }
            }
        })
        // Count salaries
        if (document.querySelector(".expensesSalaries") !== null) {
    ...
    sawroomDB.collection("dashboard").doc("operators").get().then(function(doc){
        ...
        if(element("#netPrice") && element("#grossPrice") && element("#expensesPrice") !== null) {
...
const loadDataCalendar = (data) => {
    ...
    <div class="calendarTable">
        ...
        <div class="day">
            <div class="availableOperators"></div>
    ...
    let allAvailableOperators = document.querySelectorAll(".availableOperators");
    ...
    const sameDayHours = 1800000; // Currently 0 hours
    ...
    for (let i = 0; i < allDates.length; i++) {
        ...
        setTimeout(() => {
            sawroomDB.collection("dashboard").doc((parseInt(allAvailableOperators[i].nextElementSibling.nextElementSibling.nextElementSibling.children[0].attributes[1].value*1000)).toString()).get().then(function(doc) {
                if(doc.exists) {
                    allAvailableOperators[i].innerHTML = doc.data().operators;
                } else {
                    allAvailableOperators[i].innerHTML = `<i class="fas fa-plus"></i> Add Operators`;
                }
            })
        }, 10);
    ...
    allAvailableOperators.forEach(field => {
        field.addEventListener("click", () => {
            sawroomDB.collection("dashboard").doc((parseInt(field.nextElementSibling.nextElementSibling.nextElementSibling.children[0].attributes[1].value*1000)).toString()).get().then(function(doc) {
                if(doc.exists) {
                    sawroomDB.collection("dashboard").doc("operators").get().then((doc) => {
                        const operatorColors = doc.data().operatorColors;
                        const date = new Date(field.nextElementSibling.nextElementSibling.nextElementSibling.children[0].attributes[1].value*1000);
                        const confirmDialog = document.querySelector(".confirmDialog");
                        confirmDialog.style.display = "flex";
                        confirmDialog.innerHTML = `
                            <div class="container">
                                <div class="heading">Operators For ${new Intl.DateTimeFormat('en-GB').format(date)}</div>
                                <div class="calendar">
                                    <div class="editTeamOperatorsAndColors">${doc.data().allOperatorsDiv}</div>
                                    </div>
                        ...
                        for (let i = 0; i < operatorColors.length; i++) {
                            document.querySelectorAll(".inputColor")[i].value = operatorColors[i];
                            document.querySelectorAll(".inputColor")[i].setAttribute("disabled", "");
                        }   
                        for (let i = 0; i < document.querySelector(".editTeamOperatorsAndColors").childNodes.length; i++) {
                            if (document.querySelector(".editTeamOperatorsAndColors").childNodes[i].textContent == ",") {
                                document.querySelector(".editTeamOperatorsAndColors").childNodes[i].textContent = ""
                        ...
                        document.querySelectorAll(".deleteButton").forEach((button) => {
                            button.innerHTML = "<i style=\"padding: 3px\" class=\"fas fa-check\"></i>"
                        })
                        var memberArray = [];
                        document.querySelectorAll(".teamMember").forEach((member) => {
                            member.addEventListener("click", () => {
                                if (member.children[1].innerHTML == "<i style=\"padding: 3px\" class=\"fas fa-check\"></i>") {
                                    member.children[1].innerHTML = "<i style=\"border-radius: 50%; padding: 3px;background: white; color: green;\" class=\"fas fa-check\"></i>"
                                    memberArray.push(member.children[0].textContent);
                                } else {
                                    memberArray.pop(member.children[0].textContent)
                                    member.children[1].innerHTML = "<i style=\"padding: 3px\" class=\"fas fa-check\"></i>"
                                }
                            })
                        })
                        document.querySelector(".accept").addEventListener("click", () => {
                            field.textContent = memberArray;
                            sawroomDB.collection("dashboard").doc((parseInt(field.nextElementSibling.nextElementSibling.nextElementSibling.children[0].attributes[1].value*1000)).toString()).update({
                                "operators": memberArray,
                            }).then(function() {
                    ...
                    sawroomDB.collection("dashboard").doc("operators").get().then((doc) => {
                        ...
                        document.querySelector(".accept").addEventListener("click", () => {
                            field.textContent = memberArray;
                            sawroomDB.collection("dashboard").doc((parseInt(field.nextElementSibling.nextElementSibling.nextElementSibling.children[0].attributes[1].value*1000)).toString()).set({
                                "operators": memberArray,
                            }).then(function() {
                    ...
                    if(doc.exists && booking.classList.contains("booked")) {
                        ...
                        if(enableBlocking.textContent == "Enable Blocking" && booking.classList.contains("free")) {
                            ...
                            sawroomDB.collection("dashboard").doc("operators").get().then(function(doc){
                                ...
                                firstOperator.className = "operator firstOperator";
                                firstOperator.appendChild(defaultOption);
                                for (let i = 0; i < allOperators.length; i++) {
                                    let option = document.createElement("option");
                                    option.textContent = allOperators[i];
                                    option.id = "op1";
                                ...
                                secondOperator.className = "operator secondOperator";
                                secondOperator.appendChild(defaultOption);
                                for (let i = 0; i < allOperators.length; i++) {
                                    let option = document.createElement("option");
                                    option.textContent = allOperators[i];
                                    option.id = "op2";
